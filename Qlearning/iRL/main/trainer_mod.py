...
class Trainer:
    ...
    def simulate(
        ...
        while True:
            ...
            if is_eval is True:
                if timestep % eval_interval == 0:
                    ...
                    # ------------------------------ Q値観察の改良 ここから
                    vals = self.agt.get_Q(np.array(self.monitor_x))
                    if vals is None:
                        vals = np.array([0, 0])
                        print('vals is None')
                    self.hist_Qs.append(vals.tolist())
                    # ------------------------------ Q値観察の改良 ここまで
        ...
        # シミュレーション終了後にQ値を表示
        self._show_Q()
        self.hist_Qs = np.array(self.hist_Qs)  # <---------- Q値観測の改良　追加
    ...
    def save_history(self, pathname):
        ...
        np.savez(
            ...
            eval_Qs=self.hist_Qs,  # <---------- Q値観測の改良　追加
    ...
    def load_history(self, pathname):
        ...
        self.hist_Qs = hist['eval_Qs']  # <---------- Q値観測の改良　追加
